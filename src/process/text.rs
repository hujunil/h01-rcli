...
pub fn process_text_sign(
    ...
    let signer: Box<dyn TextSigner> = match format {
        TextSignFormat::Blake3 => Box::new(Blake3Signer::try_new(key)?),
...
pub fn process_text_key_generate(format: TextSignFormat) -> Result<HashMap<&'static str, Vec<u8>>> {
    match format {
        TextSignFormat::Blake3 => Blake3Signer::generate(),
...
mod tests {
    ...
    #[test]
    fn test_process_text_blake3_sign_and_verify() {
        let mut reader = "hello".as_bytes();
        let mut reader1 = "hello".as_bytes();
        let signature = process_text_sign(&mut reader, KEY, TextSignFormat::Blake3).unwrap();
        let signature_str = URL_SAFE_NO_PAD.encode(&signature);
        println!("signature: {}", signature_str);

        let verified =
            process_text_verify(&mut reader1, KEY, &signature, TextSignFormat::Blake3).unwrap();
        assert!(verified);
    }

    #[test]
    fn test_process_text_ed25519_sign_and_verify() {
        let sk = include_bytes!("../../fixtures/ed25519.sk");
        let pk = include_bytes!("../../fixtures/ed25519.pk");
        let mut reader = "hello".as_bytes();
        let mut reader1 = "hello".as_bytes();
        let signature = process_text_sign(&mut reader, sk, TextSignFormat::Ed25519).unwrap();
        let signature_str = URL_SAFE_NO_PAD.encode(&signature);
        println!("signature: {}", signature_str);
        let verified =
            process_text_verify(&mut reader1, pk, &signature, TextSignFormat::Ed25519).unwrap();
        assert!(verified);
    }
}
