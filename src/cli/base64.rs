...
pub enum Base64SubCommand {
    #[command(name = "encode", about = "Encode input string to base64")]
    Encode(Base64EncodeOpts),
    #[command(name = "decode", about = "Decode base64 string to original")]
...
impl CmdExecutor for Base64EncodeOpts {
    async fn execute(self) -> anyhow::Result<()> {
        ...
        let ret = crate::process::b64::process_encode(&mut reader, self.format)?;
        println!("{}", ret);
        Ok(())
    }
}
...
impl CmdExecutor for Base64DecodeOpts {
    async fn execute(self) -> anyhow::Result<()> {
        let mut reader = crate::get_reader(&self.input)?;
        let ret = crate::process::b64::process_decode(&mut reader, self.format)?;
        println!("{}", ret);
        Ok(())
    }
}
